...
public class DowLoadProgress {
	...
	public  DowLoadProgress(Context context) {

	  this.mContext=context;
	  
	  //创建浮动窗口
	   creatFloatView();		
		mDownLoadController=DownLoadController.getInstance();
		...
		handler = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				case ACTION_FILE_DOWNLOAD:				
					...
					fileType=convertHttpURLToFileUrl(fileType);
					fileUrl=convertHttpURLToFileUrl(fileUrl);

					if (null != fileType && null != fileUrl) {
						// 设置下载标题：
						setDownLoadTitle(fileType, fileUrl );
						if(null == fileUtil){
							fileUtil=new FileUtil();
						}
						mDownLoadController.gotoDownload(this,fileUtil, fileType, fileUrl);
					}
					break;
				case FILE_DOWNLOAD_OK:
					//如果是闹铃音乐，通知闹铃设置，文件路径	
					if(fileType.equals("clock")){
				...
				case FILE_DOWNLOAD_ERROR:
					// 文件现在失败提示
					showResult(fileType+"下载失败");
				case FILE_IS_EXIST:
					// 文件现在失败提示
					showResult(fileType+"已存在");						
					break;
				case ACTION_EXIT:
					// 退出文件下载
