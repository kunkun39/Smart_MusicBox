...
public class XiamiMusicListActivity extends BaseActivity {
	...
	private final int MUSIC_TYPE_ALBUM = 1;// 专辑音乐类型
	private final int MUSIC_TYPE_SCENE = 2; // 场景音乐
	private final int MUSIC_TYPE_COLLECT = 3;// 精选集音乐类型
	private final int MUSIC_TYPE_TODAY = 4;// 今日推荐歌曲列表

	private final int MUSIC_LIST_UPDATE = 6; // 更新音乐列表
	...
	private Handler mhandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case MUSIC_TYPE_SCENE: // 场景音乐
				...
				params.put("limit", 20);
				params.put("page", 1);
				FindSongTask findSongByIdTask = new FindSongTask(
						getApplicationContext(), "tag.song");
				findSongByIdTask.execute(params);
				break;

			case MUSIC_TYPE_TODAY:// 今日推荐歌曲列表
...
//		if (MUSIC_TYPE_ALBUM == type||MUSIC_TYPE_TODAY == type) {
			getAlbumList();
//		}  else {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				if (curMusicType == MUSIC_TYPE_ALBUM) {
					ArrayList<OnlineAlbum> albumList = (ArrayList<OnlineAlbum>) mXMMusicData
							.getNewAlbums(LanguageType.huayu, 10, 1);
					album = albumList.get(albumIndex);
					albumID = album.getAlbumId();
					// 根据ID获取专辑相信信息，带歌曲列表
					album = XMMusicData
							.getInstance(XiamiMusicListActivity.this)
							.getDetailAlbum(albumID);
					songsList = album.getSongs();
					curTitle = album.getAlbumName();

				} else if (curMusicType == MUSIC_TYPE_SCENE) {
					OnlineCollect mOnlineCollect = mXMMusicData
							.getCollectDetailSync((int) albumID);
			...
			musicsList.post(new Runnable() {
				...
				public void run() {
					if (null != songsList) {
						...
						Toast.makeText(XiamiMusicListActivity.this,
								"没有搜索到专辑信息", Toast.LENGTH_SHORT).show();
					}
				}
			});
		}
