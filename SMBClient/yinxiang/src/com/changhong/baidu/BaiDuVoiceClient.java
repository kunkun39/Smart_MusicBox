...
public class BaiDuVoiceClient {
	...
	public BaiDuVoiceClient(Context context) {
	...
	private void initBaiduConfiguration() {
		if (recognitionClient == null) {
			recognitionClient = VoiceRecognitionClient.getInstance(mContext);
	...
	public void startVoiceRecognition() {
		...
		if (code != VoiceRecognitionClient.START_WORK_RESULT_WORKING) {
			Toast.makeText(mContext, "网络连接出错，请重新尝试", Toast.LENGTH_LONG).show();
		}
	}
	...
	public class BaiDuVoiceChannelDialogRecogListener implements
		...
		public void onError(int errorType, int errorCode) {
			Toast.makeText(mContext, "抱歉哟，我们不能识别空指令", Toast.LENGTH_LONG).show();
			isRecognitioning = false;
			recognitionClient.stopVoiceRecognition();
		}
		...
		private void updateRecognitionResult(Object result) {
			...
			if (StringUtils.hasLength(recognitionResult)) {
				...
				if (StringUtils.hasLength(commands)) {
					...
					hasResult = true;
					Toast.makeText(mContext, "语音的结果为:" + recognitionResult,
			...
			if (!hasResult) {
				...
				if (recognitioningFailedTimes == 3) {
					...
					Toast.makeText(mContext, "语音搜索:" + recognitionResult,
