 ...
 * @see com.nostra13.universalimageloader.core.ImageLoaderConfiguration
...
final class LoadAndDisplayImageTask implements Runnable, IoUtils.CopyListener {
	...
	private final boolean isSmallImageGenerate;
	...
	public LoadAndDisplayImageTask(ImageLoaderEngine engine, ImageLoadingInfo imageLoadingInfo, Handler handler) {
        ...
        isSmallImageGenerate = options.isSmallImageGenerate();
    }
	...
	private Bitmap tryLoadBitmap() throws TaskCancelledException {
		...
		try {
			...
			if (bitmap == null || bitmap.getWidth() <= 0 || bitmap.getHeight() <= 0) {
                ...
                if (isSmallImageGenerate) {
                    String smallBitmapPath = DiskCacheFileManager.isSmallImageExist(imageUriForDecoding);
                    if (!smallBitmapPath.equals("")) {
                        smallBitmapPath = Scheme.FILE.wrap(smallBitmapPath);
                        bitmap = decodeImage(smallBitmapPath);
                    } else {
                        bitmap = decodeImage(imageUriForDecoding);
                        final Bitmap convert = bitmap;
                        Thread thread = new Thread() {
                            @Override
                            public void run() {
                                DiskCacheFileManager.saveSmallImage(convert, uri);
                                DiskCacheFileManager.checkMaxFileItemExceedAndProcess();
                            }
                        };
                        thread.start();
                    }
